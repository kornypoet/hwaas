...
Vagrant.configure('2') do |config|
  config.vm.define :redis do |agent|
    ...
    agent.vm.hostname = 'redis-server'

    agent.vm.provider :virtualbox do |vbox|
      vbox.memory = 1024
    end

    agent.vm.provision :file, source: local_yaml, destination: '/tmp/common.yaml'
    ...
    agent.vm.provision :shell, name: 'packages', inline: <<-SCRIPT
      ...
      mv /tmp/common.yaml '/etc/puppetlabs/code/environments/production/hieradata/common.yaml'
    SCRIPT
  ...
  config.vm.define :hwaas do |agent|
    ...
    agent.vm.provision :shell, name: 'packages', inline: <<-SCRIPT
      ...
      dpkg -i /usr/local/src/puppetlabs-release-pc1-xenial.deb
      apt-get update
      apt-get install -y python python-setuptools puppet-agent
